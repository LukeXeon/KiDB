...
// cmdcounter.go：Client 命令计数包装（断言"覆盖路径零回表"/"投影走 HMGET"
...
// NewCmdCounter 包装。
func NewCmdCounter(inner kv.Client) *CmdCounter {
	return &CmdCounter{Client: inner, counts: map[string]int{}}
}
...
func (c *CmdCounter) Do(ctx context.Context, cmd string, args ...any) (any, error) {
	...
	c.mu.Unlock()
	return c.Client.Do(ctx, cmd, args...)
}

// Pipeline 逐命令计数并透传。
func (c *CmdCounter) Pipeline(ctx context.Context, cmds []kv.Cmd) ([]any, error) {
	c.mu.Lock()
	...
	c.mu.Unlock()
	return c.Client.Pipeline(ctx, cmds)
}
