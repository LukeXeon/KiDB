...
// 可用的 kv.Client（docs/12 §12.2：单元/PBT 层跑在 miniredis 上）。
package testutil
...
func New(t testing.TB) (kv.Client, *script.Registry, *miniredis.Miniredis) {
	t.Helper()
	m := miniredis.RunT(t)
	addr := m.Addr()
	...
	cli := kv.NewRetryingClient(raw, kv.DefaultRetryPolicy())
...
func ServerClock(cli kv.Client) func() time.Time {
