...
> **v6.x review 修复项**：
> - `CREATE UNIQUE INDEX` 非空表：先同步存量查重（流式去重，重复 1062 拒建；
>   distinct 超 `l2_max_collect` 拒建），回填作业为存量行补建唯一预约；
>   回填期预约冲突（查重后与并发写入交错产生真实重复）→ 作业中止 + 索引定义回滚
>   （告警 + 指标，MySQL 在线 DDL 收尾失败同款语义）；
> - DROP 清理作业在途的同名表 CREATE 拒绝（1235）——旧作业与旧代行同 key 命名空间，
>   撞名重建会把旧作业永久卡死且幽灵行对新表可见（实证）。
