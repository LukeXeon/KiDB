...
- **自研理由**：L1 的第一纪律是"过期正确性由读路径兜底"（Get 时一次整数比较，超 TTL 不返回）；现成库的过期语义普遍是"ticker 清扫前仍返回过期值"，不满足该纪律。且 L1 在 scatter 热路径上要求分片并发。结构极小（128 分片 × RWMutex+map，周期清扫协程），自研成本低于适配成本；
- **结构**：FNV-1a 分片；条目 = `{val, deadline}`；容量按分片均摊，超限随机驱逐（TTL 3s 下过期压力主导，驱逐极少触发）；
- **职责分离**：过期正确性由读路径兜底（Get 时一次整数比较）；清扫协程只负责释放内存（逐分片写锁删过期项，间隔 min(TTL/4, 100ms)）——删除与 Add 同在分片写锁内，无 TOCTOU；
- 全扫结果的指纹不入缓存；单测覆盖：过期读路径拦截、Replace 无扰、并发对撞排空（-race）、容量上限、退出无泄漏。
