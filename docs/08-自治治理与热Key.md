...
| 层 | 机制 | 实现 |
|---|---|---|
| L1 | 近缓存 | **分片 map + 值内 deadline + 周期清扫**（v2 重设计，见下注）：谓词指纹→pk 列表 + BucketMap 缓存，TTL 3s；**全扫结果的指纹不入缓存**；回表校验兜底陈旧 |
...
**L1 v2 重设计（分片 map，替代 ARC+PQ+janitor）**：

- **动机（基准验证）**：HashiCorp ARC 全结构一把互斥锁（Get 也抢锁），叠加外层互斥与 PQ 双结构维护——8:1 读写并行混合负载下 243.7ns/op；分片 map 方案 46.4ns/op（**5.3×**，Apple M1 Max）。ARC 自适应抗扫描的价值被"全扫指纹不入缓存"规则对冲，边际≈0；
- **结构**：128 分片 × `RWMutex + map[key]entry{val, deadline}`；FNV-1a 分片；容量按分片均摊，超限随机驱逐（TTL 3s 下过期压力主导，驱逐极少触发）；
- **职责分离不变**：过期正确性由读路径兜底（Get 时一次整数比较）；清扫协程只负责释放内存（逐分片写锁删过期项，间隔 min(TTL/4, 100ms)）；
- **防误删纪律的结构化解**：map 即唯一结构，清扫删除与 Add 同在分片写锁内——v1 的"弹出比对 + 互斥临界区"舞蹈整个消失，无 TOCTOU 可能；
- 单测覆盖：过期读路径拦截、Replace 无扰、并发对撞排空（-race）、容量上限、退出无泄漏。
