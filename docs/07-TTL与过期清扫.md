...
**容量账与细分机制（v7.0 缓存实际口径）**：集中化后登记册成员数 = 表行数（不再除以 16384）。
按每成员约 100B（pk + skiplist/dict 开销）估算：**~5 万行 = 5 万成员（桶纪律阈值）；
~80 万行 = 8MB（slot 迁移红线）**。缓存场景的真实规模（docs/01 §1.1 定位）：绝大多数表
在万~十万行级——**永远 1 册，细分机制在其生命周期内不会触发**；大缓存表（百万行+，
罕见）由登记册细分（`exp:{table}#{n}`，按 pk 散列选册）兜底——与桶分裂同纪律的
自治项：超 5 万成员/8MB 或册写热由 Controller 自动细分（keycodec 分片键机制保留，
**不设用户申报字段**，docs/01 §1.0）；细分为 2 的幂，COUNT(*) = n 个 ZCOUNT、
全表遍历 = n 册分页，语义不变（n=32 时 COUNT 为 32 命令，仍远小于 v6 的 16384）。
超限前功能始终正确（`ZCOUNT`/`ZRANGE` 分页均为 O(log N + 批大小)），8MB 是有界性
承诺线不是正确性线。写热点同理：单册 ZADD 集中在一跳，细分触发线同时吸收写吞吐集中
（同桶分裂的"热值集中"纪律，[03](03-数据模型与编码.md) §3.3）。
