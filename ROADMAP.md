...
## v7.0 索引槽位独立化（✅ 阶段 0~6 已落地 2026-08-17，全测试绿；仅剩阶段 7 真实集群门禁——外部环境项）

> 触发（用户裁决，2026-08-16）：一致性目标从"行+索引原子可见"降为 **Redis 级**
>（缓存语义：正常路径零窗口、故障路径有界收敛以 miss 呈现、查询精确性不放），
> 换 16384 穷举扇出消亡；**追加裁决二**：取消 `ReadWriteOnly` 纯读写手动豁免开关——
> 所有节点必须参与后台角色竞选，忙节点自动退让（退避式竞选）、闲节点自动接管
>（设计稿 §9）；**追加裁决三**：消除所有非泛型数据结构容器——全仓审计后库选型
> 裁决为零新依赖（sort→slices×6、utils.Set[K] 类型化×24、堆已泛型化；网络调研
> 记录见设计稿 §10）。设计推导稿 = [docs/v7.0-索引槽位独立化.md](docs/v7.0-索引槽位独立化.md)
>（含版本戳 member 不漏行不变式证明、key 布局对照表、代价诚实清单）。
> 实施阶段划分见该文档 §13。**落地状态**：阶段 0（docs 正式改写）✅、阶段 1~4
>（keycodec/bm/member 版本戳 + 两段写 + 读路径 + 分裂/L4/清扫/对账/回填/DROP 全链路）✅、
> 阶段 5（角色自适应 + 空窗三件套）✅、阶段 6（非泛型容器消除）✅——全测试绿（21 包），
> 实测等值 -235×/范围 TopK -81×（docs/12 §12.7）；阶段 7（真实集群性能门禁）待外部环境。
> **总原则（用户裁决）**：v7.0 实施不得为兼容旧路径放弃破坏性修改与最优设计——
> 无过渡形态、无兼容层、无新旧双跑（沿 v6.0"当新项目做"纪律）。
> 已裁决项（2026-08-16，AskUserQuestion 四项）：开工方式 = 阶段 6 先行+文档批同步；
> 预分裂 = 否（自动分裂承接）；登记册细分阈值 = 不预置（规模红线自动项）；
> 代价语义（全员忙空窗 / 写入集中窗口）= 接受。
> **追加裁决四（空窗兜底三件套，一次做完）**：①空窗上界 300s（退避竞选兜底闸，
> 锁无人接管超上界则无视退避强制竞选）；②唯一预约 TTL 自愈（24h PX + 过期接管
> Lua）；③补写死信队列（`dlq:idx:{table}:{idx}` + 指标）。不做：L1 延寿、全局
> inflight 闸、单桶紧急通道、本地兜底存储。常数全内置零新开关，并入阶段 5。
...
| 性能基准门禁（k6/自研压测，1 千万行数据集——缓存实际大表口径） | docs/12 §12.7 指标在案；内核侧基线基准已落地（exec/bench_test.go + shape_test.go 命令形状不变式），缺真实集群门禁 |
| exp 登记册自动细分（体积超阈自动重散列，docs/07 §7.2 容量账） | 分片键机制保留在 keycodec；当前恒 1 分片。**v7.0 口径**：集中单册后细分 = 与桶分裂同纪律的自治项（5 万成员/8MB 触发，缓存实际下绝大多数表永不触发，百万行+ 大表出现前落地即可）。设计取向定格：表级状态 `esm:{table}`={active,next,cursor}，按序迁移，读侧按游标选布局，游标边界迁移期写双写覆盖并发竞态（对齐桶分裂 SPLITTING 双写模式）。触发条件：Controller 巡检登记册 ZCARD/体积超阈。**前置依赖**：百万行级真实负载或测试数据集就位，否则机制空转无验证面 |
