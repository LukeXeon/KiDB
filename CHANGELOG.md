...
**进度**：设计稿 + 四项开放问题裁决 ✅；阶段 0（docs 正式改写，三批次）✅；
阶段 6（触发三容器消除）✅；**阶段 1~5 全部落地（2026-08-17，全测试绿 21 包）**——
两段写 + 版本戳 member + `_iv` 创建版本字段；扇出消亡实测：等值 -235×、范围 TopK -81×
（docs/12 §12.7）；角色自适应 + 空窗三件套 + ReadWriteOnly 拆除 ✅。
**实施期抓获并修复**（全部有回归测试钉死）：并发分裂迟到写丢行（drainGrace 排干宽限，
203/198 实证）；版本戳下无条件 redo 的确定性垃圾累积（撤/建值变化条件化）；member 创建
版本 ≠ 行版本（_iv 字段补全撤销精度）；预约 24h PX 时间戳破长命行唯一约束（唯一桶复核）；
reserveUnique 双重转义；v6 潜伏的 SplitEq 转义值双重转义。阶段 7（真实集群门禁）待外部环境。
