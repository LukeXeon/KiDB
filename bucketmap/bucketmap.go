...
type Store struct {
	...
	regCache map[string]utils.Set[string] // 热值注册表缓存（同 TTL——review 实证：Registry 无缓存曾让每条索引查询 +1 RTT）
}
...
func (s *Store) Registry(ctx context.Context, table, idx string) (utils.Set[string], error) {
